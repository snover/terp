...
public class TERMultiWordClassCost extends TERMultiCost {
  /* For all of these functions, the score should be between 0 and 1
   * (inclusive).  If it isn't, then it will break TERcalc! */

    static {
	COST_NAME = "Multi Word Class TERcost";
	IS_MULTI = true;
    }

    TERMultiWordClassCost() {
    ...
    TERMultiWordClassCost(String fname) {
	nameMap.put(OTHER_NAME, 0);
	revNameMap.put(0, OTHER_NAME);
	_sub_cost[0][0] = default_substitute_cost;
	_match_cost[0] = default_match_cost;
    ...
    TERMultiWordClassCost(String fname, double dsub_cost, double dmat_cost,
    ...
    private HashMap<Integer, String> revNameMap = new HashMap();
    private int numClasses = 1;
    private ArrayList<Integer> classCounts = new ArrayList(100);
	...
	return findClass(s, 0, s.length);
    }

    private int findClass(Comparable[] s, int start, int len) {
	return _findClass(s, start, len);
	//	word_class wc = (word_class) s;
	//	return wc.wclass;
    }

    private int _findClass(Comparable[] s) {	
	return _findClass(s, 0, s.length);
    }   
 
    private int _findClass(Comparable[] s, int start, int len) {	
	int tr = -1;	
	if (multitree.contains(s, start, len)) {
	    List<Object> lob = multitree.find(s, start, len);
	    tr = ((Integer) lob.get(0)).intValue();
	} else {
	    if (len == 1) {
		Matcher np1 = NUMBER_PAT1.matcher(s[start].toString());
		Matcher np2 = NUMBER_PAT2.matcher(s[start].toString());
	    ...
	    classCounts.set(tr, classCounts.get(tr) + 1);
	}
	return tr;
    }

    /*
    private int[] _findClasses(Comparable s) {	
	int[] tr = null;
	if (wordMap.containsKey(s)) {
	    tr = ((int[]) wordMap.get(s));
	} else {
	    int[] iob = null;
    ...
    */
	...
	BufferedReader stream;
		...
		if (line.equals(NUMBER_STR)) {
		    ...
		    multitree.insert(line.split("\\s+"), new Integer(classnum));
		    classCounts.set(classnum, classCounts.get(classnum) + 1);
		}
	    }	    
        } catch(IOException ioe) {
            System.out.println("load word class list: " + ioe);
  ...
  private int get_class_num(String name) {
	  ...
	  numClasses++;
	  while (classCounts.size() <= numClasses) {
	      classCounts.add(0);
	  }
      } else {
    ...
    public double match_cost(Comparable[] hyp, int hstart, int hlen,
			     Comparable[] ref, int rstart, int rlen) { 
	return _match_cost[findClass(hyp, hstart, hlen)];
    }
    ...
    public double substitute_cost(Comparable[] hyp, int hstart, int hlen,
				  Comparable[] ref, int rstart, int rlen) {
	int rc = findClass(ref, rstart, rlen);
	int hc = findClass(hyp, hstart, hlen);
	double cost = _sub_cost[rc][hc];
	//	System.out.printf("Cost of class %d sub with %d is %f\n", rc, hc, cost);
	return cost;
    }
    ...
    public double stem_cost(Comparable[] hyp, int hstart, int hlen,
			    Comparable[] ref, int rstart, int rlen) {
	return _stem_cost[findClass(ref, rstart, rlen)][findClass(hyp, hstart, hlen)];
    }
	...
	return _shift_cost[findClass(shift)];
    }
...
//     }
	...
	for (int i = 0; i < shift.shifted.size(); i++) {	  
	    int mclass = findClass((Comparable[]) shift.shifted.toArray(new Comparable[0]), i, 1);
	...
	for (int i = 0; i < tr.length; i++) 
	    tr[i] = classCounts.get(i).intValue();
	return tr;
    }
