...
public class TERcost {
  ...
  protected static boolean IS_MULTI = false;

    public static boolean is_multi() {return IS_MULTI;}
  ...
  public double shift_cost(TERshift shift) {
    ...
    public String to_out_txt(Comparable c) { return c.toString(); }
    public String to_out_txt(Comparable[] c) { 
	return to_out_txt(c, 0, c.length);
    }
    public String to_out_txt(Comparable[] c, int s, int l) { 
	String tr = "";
	for (int i = 0; i < l; i++) {
	    if (i == 0) {
		tr += c[i+s].toString();
    ...
    public String to_out_html(Comparable c) { return c.toString(); }
